...
export default function Explore() {
  ...
  const filteredProjectResults = projectResults.filter(result => new Date(result.createdAt) >= new Date('2025-03-01'));

  // Filter users who joined after January 1st, 2020
  const filteredUserResults = userResults.filter(result => {
    const createdAtDate = new Date(result.createdAt);
    const formattedDate = createdAtDate.toLocaleString('en-US', { timeZone: 'UTC' });
    return formattedDate >= '3/1/2025';
  });

  console.log('Original user results:', userResults);
  console.log('Filtered user results:', filteredUserResults);
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <Tab.Group selectedIndex={selectedTab} onChange={setSelectedTab}>
        <Tab.List className="flex border rounded-lg mb-6 bg-gray-100 divide-x divide-gray-200">
          ...
          >
            Projects ({filteredProjectResults.length})
          </Tab>
          <Tab
            className={({ selected }) =>
              clsx(
          ...
          >
            Users ({filteredUserResults.length})
          </Tab>
        </Tab.List>
        ...
        <Tab.Panels>
          ...
          >
            {isLoading ? (
              ...
              >
                {filteredProjectResults.map((result, index) => (
                  <motion.div
          ...
          >
            {isLoading ? (
              ...
              >
                {filteredUserResults.map((result, index) => (
                  <motion.div
                    key={`user-${result.id}`}
                    initial={{ opacity: 0, y: 20 }}
                    animate={{ opacity: 1, y: 0 }}
