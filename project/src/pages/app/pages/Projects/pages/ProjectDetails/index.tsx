...
export function ProjectDetails() {
  ...
  const handleJoinRequest = async (roleId: string | null, message: string, attachments: File[]) => {
    if (!user || !project) {
      console.log('Cannot submit join request:', { user, project });
      return;
    }
    ...
    try {
      ...
      console.log('Creating join request with data:', {
      ...
      console.log('Saving join request document:', joinRequest);

      // Add request to Firestore
      const docRef = await addDoc(collection(db, 'joinRequests'), joinRequest);
      console.log('Join request saved with ID:', docRef.id);
  ...
  useEffect(() => {
    if (!project?.id || !isProjectOwner) {
      console.log('Join requests subscription not active:', { 
        projectId: project?.id, 
        isProjectOwner,
    ...
    console.log('Setting up join requests subscription for project:', project.id);
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      console.log('Join requests snapshot update:', {
        numberOfRequests: snapshot.docs.length,
    ...
    return () => unsubscribe();
  }, [project?.id, isProjectOwner, user?.uid]);
              ...
              <div className="border-b border-gray-200 bg-gray-50/50 px-6 py-4">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-gray-500">View and edit your project details</p>
                  </div>
                  <Button
                    variant="outline"
                  ...
                  <div>
                    ...
                    <dl className="grid grid-cols-1 gap-6 sm:grid-cols-2">
                      ...
                      <div className="p-4 bg-gray-50/50 rounded-lg border border-gray-200">
                        ...
                        <dd className="mt-1 text-sm text-gray-900 capitalize">{project.category}</dd>
                      </div>
                      <div className="p-4 bg-gray-50/50 rounded-lg border border-gray-200">
                        <dt className="text-sm font-medium text-gray-500">Phase</dt>
                        <dd className="mt-1 text-sm text-gray-900 capitalize">{project.phase}</dd>
                      </div>
                      <div className="p-4 bg-gray-50/50 rounded-lg border border-gray-200">
              ...
              <div className="border-b border-gray-200 bg-gray-50/50 px-6 py-4">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-gray-500">Configure project roles and requirements</p>
                  </div>
                  <Button
                    variant="outline"
              ...
              <div className="divide-y divide-gray-200">
                {project.requiredRoles?.map((role) => (
                  <div key={role.id} className="p-6">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-4">
                        <div className={`w-10 h-10 rounded-lg bg-${role.color || 'gray'}-50 flex items-center justify-center border border-${role.color || 'gray'}-100`}>
                          <svg className={`w-5 h-5 text-${role.color || 'gray'}-600`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 13.255A23.931 23.931 0 0112 15c-3.183 0-6.22-.62-9-1.745M16 6V4a2 2 0 00-2-2h-4a2 2 0 00-2 2v2m4 6h.01M5 20h14a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                          </svg>
                        </div>
                        <div>
                          ...
                          {role.skills && role.skills.length > 0 && (
                            <div className="mt-2 flex flex-wrap gap-1.5">
                              {role.skills.map((skill) => (
                                <span
                                  key={skill}
                                  className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-${role.color || 'gray'}-50 text-${role.color || 'gray'}-700 border border-${role.color || 'gray'}-200`}
                                >
                                  {skill}
                                </span>
                              ))}
                ...
                {(!project.requiredRoles || project.requiredRoles.length === 0) && (
                  <div className="text-center py-12">
                    ...
                    <p className="text-sm text-gray-500 max-w-sm mx-auto mb-4">
                      Define roles to specify the skills and responsibilities needed for your project.
                    </p>
                    <Button
                      variant="outline"
                      onClick={() => setShowAddRoleModal(true)}
                        ...
                        <div className="mt-4 space-y-4">
                          <div className="p-4 bg-red-50/50 rounded-lg border border-red-100">
                            <label htmlFor="confirm-delete" className="block text-sm font-medium text-gray-700 mb-2">
                              To confirm deletion, type "{project.title}" below:
                            </label>
                            <input
                              type="text"
                              id="confirm-delete"
                              ...
                              className="block w-full rounded-lg border border-red-200 px-3 py-2 focus:border-red-500 focus:ring-red-500 sm:text-sm bg-white"
                          ...
                          <Button
                            ...
                            disabled={deleteConfirmText !== project.title || !canDeleteProject}
                          ...
                          {!canDeleteProject && (
                            <div className="flex items-center gap-2 mt-2 text-sm text-red-600">
                              ...
                              Project cannot be deleted while it is in a closed state.
                            </div>
                          )}
                        </div>
                      </div>
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-8">
      ...
      {/* Hero Section */}
        ...
        <div className="px-10 py-12 space-y-12">
          ...
          {/* Join Requests Section - Below About & Team */}
          {isProjectOwner && joinRequests.length > 0 && (
            <div className="grid grid-cols-1 gap-3">
              <Card className="overflow-hidden border border-gray-200">
                <div className="border-b border-gray-200 bg-gray-50/50 px-6 py-4">
                  <div className="flex items-center justify-between">
                    <div>
                      <h2 className="text-base font-semibold text-gray-900">Join Requests</h2>
                      <p className="mt-1 text-sm text-gray-500">
                        {joinRequests.length} requests awaiting your approval
                      </p>
                    </div>
                  </div>
                </div>
        ...
        {hasProjectAccess && (
          ...
          <div className="mt-8">
            ...
            {isProjectOwner && renderJoinRequests()}
